...
async def test():
    ...
    print("Deleting ultron.db...")
    if db_file.is_file():
        try:
            os.remove(db_file)
            print("ultron.db successfully DELETED.")
        except Exception as e:
            print("Error deleting ultron.db:", e)
            return

    print("Running init_db() on clean database...")
    await init_db()

    async with AsyncSessionLocal() as db:
        res = await db.execute(select(Station))
        stations = res.scalars().all()
        print(f"Total stations in clean DB: {len(stations)}")
        for s in stations:
            ...
            print("  - Pydantic StationOut validation SUCCESSFUL:", st_out.name)
