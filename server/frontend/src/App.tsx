...
function App() {
  ...
  return (
    <div className="min-h-screen bg-slate-900 font-sans text-slate-200 overflow-hidden flex flex-col relative">
      {/* Background decoration - simple gradient, no external requests */}
      <div className="absolute inset-0 bg-gradient-to-br from-slate-900 via-slate-800 to-teal-900/40 z-0" />
      
      {/* Top Header */}
      <header className="relative z-10 flex items-center justify-between px-6 py-3 bg-slate-900 border-b border-slate-700">
        ...
        <div className="flex-1 max-w-2xl px-4">
          <div className="relative flex items-center w-full h-12 rounded-full bg-slate-800/80 hover:bg-slate-700/80 focus-within:bg-white focus-within:text-slate-900 transition-colors px-4 border border-slate-600/50 shadow-inner">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-slate-400 focus-within:text-slate-600 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className="relative z-10 flex flex-1 overflow-hidden">
        ...
        <aside className="w-64 flex flex-col py-4 px-3 gap-2">
          ...
          <nav className="flex flex-col gap-1">
            {['All Sites', 'Online', 'Offline'].map(category => (
              ...
              >
                <span>{category}</span>
                {category === 'All Sites' && <span className="text-xs bg-slate-700/50 px-2 py-0.5 rounded-full">{sites.length}</span>}
        ...
        <main className="flex-1 bg-slate-800/60 border border-slate-700 rounded-tl-xl shadow-lg overflow-hidden flex flex-col mr-2 mt-2 mb-2">
            ...
            {filteredSites.length === 0 ? (
              ...
              <div className="flex flex-col">
                {filteredSites.map(site => (
                  <div key={site.id} onClick={() => setActiveSite(site)} className={`group flex items-center px-4 py-2 border-b border-slate-700/50 hover:bg-slate-700/40 transition-colors cursor-pointer text-sm ${activeSite?.id === site.id ? 'bg-teal-900/20 border-l-2 border-l-teal-500' : ''}`}>
                    {/* Left Actions */}
                    <div className="flex items-center gap-3 w-16 text-slate-500">
                      <input type="checkbox" className="rounded border-slate-600 bg-slate-800 text-teal-500 focus:ring-teal-500/50 cursor-pointer" />
                    ...
                    <div className="flex-1 grid grid-cols-12 items-center gap-4">
                      <div className="col-span-3 font-bold text-slate-200 truncate">{site.name}</div>
                      
                      <div className="col-span-3 flex flex-col gap-0.5">
                        <span className="text-slate-400 truncate flex items-center gap-2">
        ...
        {activeSite && (
          <aside className="w-96 flex flex-col bg-slate-800 border-l border-slate-700 shadow-xl overflow-hidden mt-2 mb-2 mr-2 rounded-xl">
            {/* Panel Header */}
            <div className="flex items-center justify-between px-5 py-4 border-b border-white/10 bg-black/30">
              <div>
                ...
                <p className="text-xs text-slate-400">{activeSite.location || 'Unknown Location'}</p>
              </div>
              <button
                onClick={() => { setActiveSite(null); setLiveData([]); }}
                className="p-1.5 rounded-full hover:bg-slate-700/60 text-slate-400 hover:text-white transition-colors ml-2 flex-shrink-0"
              >
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div className="flex items-center gap-2 px-5 py-2 bg-teal-900/20 border-b border-teal-800/30">
              ...
              {liveDataLoading && <span className="ml-auto text-xs text-slate-500 animate-pulse">Fetching...</span>}
            </div>
                      ...
                      return (
                        <tr key={pt.tag_name} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                          <td className="px-4 py-2">
                            ...
                            {pt.name !== pt.tag_name && <div className="text-slate-500 truncate max-w-[130px]">{pt.name}</div>}
                          </td>
                          <td className="px-3 py-2 text-right font-bold text-white">
                            {pt.value !== null && pt.value !== undefined ? pt.value.toFixed(2) : '—'}
                            {pt.unit && <span className="ml-1 text-slate-400 font-normal">{pt.unit}</span>}
                          </td>
                          ...
                          <td className="px-3 py-2 text-right text-slate-500">{ago} ago</td>
                        </tr>
                      );
                    })}
                  </tbody>
